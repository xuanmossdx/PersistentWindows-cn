...
if not errorlevel 1 goto wait_to_finish";
        ...
        static void Main(string[] args)
        {
            ...
            string redirect_appdata = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "user_data"); // always store db file under the exe folder, regardless of command-line args
        ...
        static public void CaptureSnapshot(int id, bool prompt = true, bool delayCapture = false)
        {
            ...
            snapshot_timer = new System.Threading.Timer(state =>
            {
                ...
                if (!silent)
                {
                    ...
                    if (prompt)
                        systrayForm.notifyIconMain.ShowBalloonTip(5000, $"快照 '{c}' 已保存", $"点击图标后立即按数字键 '{c}' 即可恢复该快照", ToolTipIcon.Info);
                }
