...
托盘菜单、气泡通知、启动画面、各类弹窗的用户可见文本全部替换为简体中文。

> 为使菜单状态切换逻辑与中文菜单文字匹配，同步修改了 `SystrayForm.cs` 中 4 处以菜单文字内容作为状态判断的 `Contains(...)` 条件。全部文本替换对照见根目录的 `translate.pl`（同时是一个可重放的翻译脚本，上游出新版本后可对新代码重跑一次再编译）。
