...
全部用户可见文本（托盘菜单、气泡通知、启动画面、各类弹窗，约 45 组词条）已迁移到 `Common/Lang.cs` 的双语查表机制 `Lang.T("English", "简体中文")`：

- **默认英文**，与上游行为完全一致，英文用户无感知；
- 右键托盘图标 → **语言 / Language** 一键切换，选择持久化保存在 `user_data/lang.txt`，重启程序后全局生效；
